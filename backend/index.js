...
app.post('/createcourse', jsonParser, authenticateToken, async (req, res) => {
  ...
  try {
    ...
    //user validation
    const validation = await con.query(`select name, year from courses where name = ? and year = ?`, [requestbody.name, requestbody.year]);
    if (validation[0].length < 1) {
      //user creation
...
app.get('/getallcourses', authenticateToken, async (req, res) => {
  startvalue = (req.query.startvalue - 1) * req.query.endvalue;
  endvalue = (req.query.startvalue * req.query.endvalue) - 1;
  try {
    const [data] = await con.execute(`select * from courses LIMIT ${startvalue},${endvalue}`);
    // data.forEach( (row) => {
    //   console.log(`${row.id} = ${row.lastname} ${row.firstname} - ${row.email}`);
    // });
...
app.get('/getusercourse', authenticateToken, async (req, res) => {
  try {
    const [data] = await con.execute(`select distinct c.* from users_roles_courses urc inner join courses c on urc.id_course=c.id where urc.id_user = ? LIMIT 1`, [req.user.userid]);
    res.json(data);
  } catch (err) {
    console.log("Getuser Error:" + err);
    res.json({ error: true, errormessage: "GENERIC_ERROR" });
